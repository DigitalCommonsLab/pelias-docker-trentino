...
cat format.csv | csvsql --query "SELECT x AS lat,y AS lon,civico_num AS number,civico_alf AS alfa,cap AS zipcode, Appellativo||' '||Prenome||' '||Denominazione AS street FROM stdin" > out.csv
